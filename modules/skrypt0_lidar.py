# -*- coding: utf-8 -*-
"""
SKRYPT NR 0 (v6.0): RASTER WYSOKOŚCI KORON Z KLASY 5 LIDAR

Opis:
Skrypt ten tworzy pojedynczy raster wysokościowy (nazywany też rastrem koron
drzew lub nCHM) wyłącznie na podstawie punktów Lidar sklasyfikowanych jako
wysoka roślinność (klasa 5).
- Przetwarza pliki .laz jeden po drugim, aby oszczędzać pamięć RAM.
- Używa szybkiej, zrównoleglonej funkcji Numba do rasteryzacji.
- Obszary bez punktów klasy 5 otrzymują wartość "brak danych".
"""
import os
import numpy as np
import rasterio
...
DEFAULT_CRS = "EPSG:2180" # Używany, jeśli w plikach .laz brakuje informacji o CRS

# PARAMETRY WYJŚCIOWE
TARGET_RES = 1.0 # Rozdzielczość wynikowego rastra [metry]
NODATA_VALUE = -9999 # Wartość "brak danych" w wynikowym rastrze

# ŚCIEŻKA WYJŚCIOWA
OUTPUT_FOLDER = "/content/lidar_class5_raster"
OUTPUT_RASTER_PATH = os.path.join(OUTPUT_FOLDER, "raster_koron_drzew.tif")
